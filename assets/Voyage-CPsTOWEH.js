import{r as j,j as q,a as Et}from"./index-Dg7MOBbL.js";import{M as R,O as Ft,B as Se,F as Me,S as Q,U as Ue,V as k,W as Oe,H as ze,N as At,C as Ot,a as Le,b as I,A as te,c as zt,R as Ut,d as Lt,e as Bt,L as Dt,f as kt,g as Ht,h as wt,i as It,j as Nt,k as Ne,G as ae,l as Vt,T as Ve,m as ye,n as ct,o as ue,p as W,q as jt,r as re,D as Te,P as ce,s as oe,I as Gt,t as je,u as Y,v as Qe,w as bt,x as Wt,y as Mt,z as $t,E as qt,J as Qt,K as ut,Q as Kt,X as Yt,Y as Xt,Z as Zt,_ as Jt,$ as ea,a0 as ta}from"./three-B8NjZs_w.js";const Tt={name:"CopyShader",uniforms:{tDiffuse:{value:null},opacity:{value:1}},vertexShader:`
		...
		void main() {
			...
			gl_FragColor = mix( outputColor, texel, alpha );
				...
				}`})}}fe.BlurDirectionX=new k(1,0);fe.BlurDirectionY=new k(0,1);const ca={name:"OutputShader",uniforms:{tDiffuse:{value:null},toneMappingExposure:{value:1}},vertexShader:`
  ...
  }`};function Ye(a){let e=a>>>0;return function(){e=e+1831565813|0;let s=Math.imul(e^e>>>15,1|e);return s=s+Math.imul(s^s>>>7,61|s)^s,((s^s>>>14)>>>0)/4294967296}}const X=()=>new re({color:13225944,metalness:.94,roughness:.3}),ht=()=>new re({color:15265010,metalness:.36,roughness:.52,side:Te}),pt=()=>new re({color:14263375,metalness:1,roughness:.22}),dt=()=>new re({color:11043887,metalness:.85,roughness:.58}),Ee=()=>new re({color:1974824,metalness:.5,roughness:.66}),we=()=>new re({color:10134189,metalness:.9,roughness:.38});function pa(){const a=new ae,e=12,o=[];for(let i=0;i<=22;i++){const l=i/22*e;o.push(new k(l,l*l/44))}const s=new R(new Vt(o,72),ht());s.position.y=2.2,a.add(s);const u=new R(new Ve(e,.22,8,72),X());u.rotation.x=Math.PI/2,u.position.y=2.2+e*e/44,a.add(u);for(let i=0;i<12;i++){const r=i/12*Math.PI*2,l=new R(new ye(.16,.5,e*.95),X());l.position.set(Math.cos(r)*e*.5,1.6,Math.sin(r)*e*.5),l.rotation.y=-r,a.add(l)}const n=new R(new ct(.85,3.4,16),X());n.position.y=9.6,n.rotation.x=Math.PI,a.add(n);const h=new R(new ue(1.5,24,16,0,Math.PI*2,0,Math.PI/2.2),ht());h.position.y=11.4,h.rotation.x=Math.PI,a.add(h);for(let i=0;i<3;i++){const r=i/3*Math.PI*2+.4,l=new R(new W(.12,.12,11.2,6),we());l.position.set(Math.cos(r)*e*.46,6.4,Math.sin(r)*e*.46),l.lookAt(new I(0,11.2,0)),l.rotateX(Math.PI/2),a.add(l)}const d=new ae;d.position.y=-2.6;const v=new R(new W(4.3,4.3,2.4,10),Ee());d.add(v);for(let i=0;i<10;i++){const r=i/10*Math.PI*2+Math.PI/10,l=new R(new ye(2.5,2.1,.5),i%3===0?dt():X());l.position.set(Math.cos(r)*4.35,0,Math.sin(r)*4.35),l.rotation.y=-r+Math.PI/2,d.add(l)}const A=new R(new W(4.4,4.4,.35,10),X());A.position.y=1.35,d.add(A);const E=new R(new Ve(4.45,.22,8,10),pt());E.rotation.x=Math.PI/2,E.position.y=-1.2,d.add(E);const x=new R(new ue(2.1,24,20),dt());x.position.y=-3.2,d.add(x);const _=new R(new ct(.55,1.3,12),Ee());_.position.set(0,-5.2,0),d.add(_),a.add(d);const m=new R(new W(.26,.26,12,8),we());m.rotation.z=Math.PI/2,m.position.set(-9.5,-3.4,0),a.add(m);for(let i=0;i<3;i++){const r=-13.2-i*3.3,l=new R(new W(1.05,1.05,2.8,14),Ee());l.rotation.z=Math.PI/2,l.position.set(r,-3.4,0),a.add(l);for(let b=0;b<6;b++){const f=b/6*Math.PI*2,g=new R(new ye(2.6,.08,1.5),X());g.position.set(r,-3.4+Math.cos(f)*1.4,Math.sin(f)*1.4),g.rotation.x=-f,a.add(g)}}const T=new R(new W(.22,.22,13,8),we());T.rotation.z=Math.PI/2,T.position.set(10,-3,0),a.add(T);const w=new R(new ye(2.6,2.2,2.4),Ee());w.position.set(16.6,-3,0),a.add(w),[[.75,.5],[-.75,-.4]].forEach(([i,r])=>{const l=new R(new W(.42,.5,3.1,14),X());l.rotation.x=Math.PI/2,l.position.set(16.6+r,-3+i,-2.2),a.add(l);const b=new R(new jt(.42,16),new re({color:724500,metalness:1,roughness:.1}));b.position.set(16.6+r,-3+i,-3.76),a.add(b)});const S=new ae,p=new R(new W(.1,.13,58,6),we());p.position.y=29,S.add(p);for(let i=1;i<=3;i++){const r=new R(new ye(.7,.7,.7),X());r.position.y=i*17,S.add(r)}S.position.set(-3.4,-1.5,3),S.rotation.set(.5,0,.72),a.add(S);const c=new R(new W(1.85,1.85,.16,48),pt());c.position.set(3.6,-2.6,4.2),c.rotation.set(Math.PI/2,0,.2),a.add(c);const F=new R(new Ve(1.85,.1,8,40),X());return F.position.copy(c.position),F.rotation.copy(c.rotation),a.add(F),[-1,1].forEach(i=>{const r=new R(new W(.075,.075,40,6),we());r.position.set(i*2.2,-4.4,-2),r.rotation.set(-.85,0,i*.55),r.translateY(20),a.add(r)}),{group:a,dish:s,bus:d,magBoom:S}}const ne=`
...
${pe}
}`;function se(a,e,o={}){return new Q({vertexShader:a,fragmentShader:e,uniforms:{uTime:{value:0},uOpacity:{value:1},...o},transparent:!0,forceSinglePass:!0,depthWrite:!1,blending:te})}function We(a){const e=a()*2-1,o=a()*Math.PI*2,s=Math.sqrt(1-e*e);return new I(s*Math.cos(o),e*.83,s*Math.sin(o))}function Sa(a,e){const o=[],s=[],u=[],n=[],h=a?66:125,d=new I(0,1,0),v=new I,A=new I,E=new I,x=(m,T,w,S,p)=>{A.subVectors(T,m).normalize(),E.crossVectors(A,d).normalize().multiplyScalar(w);const c=new I().crossVectors(A,E).normalize().multiplyScalar(w);for(let F=0;F<3;F++){const i=F/3*Math.PI*2,r=(F+1)/3*Math.PI*2,l=E.clone().multiplyScalar(Math.cos(i)).addScaledVector(c,Math.sin(i)),b=E.clone().multiplyScalar(Math.cos(r)).addScaledVector(c,Math.sin(r));for(const[f,g,t]of[[m,l,p],[T,l,p+.065],[T,b,p+.065],[m,l,p],[T,b,p+.065],[m,b,p]])v.copy(f).add(g),o.push(v.x,v.y,v.z),s.push(S),u.push(t)}};for(let m=0;m<h;m++){const T=We(e),w=We(e),S=.25+e()*.4,p=.19+e()*.32,c=e()*Math.PI*2,F=e(),i=55e-5+e()*.0013;let r;for(let l=0;l<=14;l++){const b=l/14,f=S+b*p,g=T.clone().multiplyScalar(f);g.addScaledVector(w,Math.sin(b*3.6+c)*.052),g.y+=Math.sin(b*8.4+c)*.014,r&&x(r,g,i*(.95-b*.6),F,b*.88),r=g,l%3===0&&n.push({center:g.clone(),heat:F})}}const _=new Se;return _.setAttribute("position",new Me(o,3)),_.setAttribute("aHeat",new Me(s,1)),_.setAttribute("aAlong",new Me(u,1)),{geometry:_,centers:n}}function Ca(a,e){const o=new ce(1,1),s=new Gt;s.index=o.index,s.attributes.position=o.attributes.position,s.attributes.uv=o.attributes.uv;const u=new Float32Array(a.length*3),n=new Float32Array(a.length),h=new Float32Array(a.length);return a.forEach(({center:d},v)=>{u.set([d.x,d.y,d.z],v*3),n[v]=.07+e()*.17,h[v]=e()}),s.setAttribute("aCenter",new je(u,3)),s.setAttribute("aSize",new je(n,1)),s.setAttribute("aSeed",new je(h,1)),s.instanceCount=a.length,s}const be=(a,e,o)=>Y.smoothstep(o,a,e),St=a=>Math.pow(Math.max(0,(a-.36)/.64),.67);function gt(a){return 34+St(Y.clamp(a,0,1))*720}function _a(a=!1){const e=new ae,o=Ye(19870223),s=new ue(1,a?64:104,a?40:64),u=[],n=(r,l,b=R)=>{const f=new b(r,l);return f.frustumCulled=!1,e.add(f),u.push(l),f},h=n(s,se(da,va,{uRadius:{value:52},uHeat:{value:0}})),d=n(new ce(1,1),se(ma,ga,{uFlash:{value:0},uHeat:{value:0}}));d.renderOrder=5;const v=[];for(let r=0;r<2;r++){const l=n(s,se(vt,mt,{uRadius:{value:0},uLayer:{value:r*1.37},uShock:{value:0}}));l.material.side=Te,l.rotation.set(r*.8,r*1.1,r*.45),v.push(l)}const A=n(s,se(vt,mt,{uRadius:{value:0},uLayer:{value:5.2},uShock:{value:1}}));A.material.side=Te;const{geometry:E,centers:x}=Sa(a,o),_=n(E,se(wa,ba,{uRadius:{value:0}})),m=n(Ca(x,o),se(xa,ya,{uRadius:{value:0}})),T=a?1700:4800,w=new Float32Array(T*3),S=new Float32Array(T),p=new Float32Array(T);for(let r=0;r<T;r++){const l=We(o).multiplyScalar(.28+Math.pow(o(),.6)*.83);w.set([l.x,l.y,l.z],r*3),S[r]=.6+Math.pow(o(),3)*2.6,p[r]=o()}const c=new Se;c.setAttribute("position",new oe(w,3)),c.setAttribute("aSize",new oe(S,1)),c.setAttribute("aSeed",new oe(p,1));const F=n(c,se(Ma,Ta,{uRadius:{value:0}}),Qe);function i(r,l,b){const f=Y.clamp(r,0,1),g=be(.21,.36,f),t=St(f),z=Math.exp(-Math.pow((f-.377)/.026,2)),y=be(.36,.435,f),P=1-be(.88,1,f)*.25;for(const O of u)O.uniforms.uTime.value=l;h.material.uniforms.uRadius.value=f<.36?Y.lerp(54+Math.sin(l*1.1)*1.1,14,g):8+Math.exp(-t*13)*22,h.material.uniforms.uHeat.value=Math.max(g,y),h.material.uniforms.uOpacity.value=f<.36?1:.72,d.quaternion.copy(b.quaternion),d.scale.setScalar(f<.36?410-g*180:230+z*1350),d.material.uniforms.uHeat.value=g,d.material.uniforms.uFlash.value=z*3,d.material.uniforms.uOpacity.value=f<.36?.75:.35+z*1.8,v.forEach((O,M)=>{O.material.uniforms.uRadius.value=(30+t*595)*(1-M*.105),O.material.uniforms.uOpacity.value=y*P*(M===0?1:.55),O.visible=y>.001}),A.material.uniforms.uRadius.value=gt(f),A.material.uniforms.uOpacity.value=y*(1-be(.52,1,f)*.7),A.visible=y>.001,_.material.uniforms.uRadius.value=30+t*655,_.material.uniforms.uOpacity.value=y*P*.67,_.visible=y>.001,m.material.uniforms.uRadius.value=30+t*655,m.material.uniforms.uOpacity.value=y*P*.8,m.visible=y>.001,F.material.uniforms.uRadius.value=28+t*760,F.material.uniforms.uOpacity.value=y*(1-be(.55,1,f)*.55),F.visible=y>.001}return{group:e,update:i,screenRadius:gt}}const Ce=`
#include <tonemapping_fragment>
#include <colorspace_fragment>
`,Pa=`
...
void main(){
  ...
  ${Ce}
    ...
    }`,fragmentShader:`varying float vAlpha;void main(){float r=length(gl_PointCoord-.5)*2.;gl_FragColor=vec4(.4,.78,1.4,exp(-r*r*5.)*vAlpha);${Ce}}`}),w=new Qe(m,T);w.frustumCulled=!1,o.add(w);let S=null,p=0;return{group:e,update(c,F,i,r=!1,l=1){i<=0&&(S=null),c<p&&(S=null),i>0&&S===null&&(S=c),p=c;const b=i>0,f=r?8:S===null?0:c-S,g=b?ee(i,0,.07)*Y.clamp(l,0,1):0,t=ee(f,.25,2.3),z=b&&!r?ee(f,1.9,2.12)*(1-ee(f,2.12,2.52)):0,y=b?ee(f,0,4.25):0,P=b?ee(f,2.08,4.25):0,O=g*ee(f,2.08,2.5);n.quaternion.copy(F.quaternion),u.scale.setScalar(.65+t*.35+z*.14),u.rotation.y=c*.12,s.scale.setScalar(1.32-ee(f,0,1.95)*.32),s.rotation.z=c*(.035+t*.065);for(const[M,D]of[[s,g*(.35+t*.65)],[u,g],[n,g*(.3+t*.7)]])M.material.uniforms.uTime.value=c,M.material.uniforms.uOpacity.value=D,M.material.uniforms.uHeat.value=t,M.material.uniforms.uFlash.value=z;for(const M of v)M.visible=P>1e-4,M.scale.set(.4+Math.sqrt(P)*.6,P,.4+Math.sqrt(P)*.6),M.material.uniforms.uTime.value=c,M.material.uniforms.uOpacity.value=O;return w.visible=P>.001,T.uniforms.uLaunch.value=r?8:f-2.08,T.uniforms.uOpacity.value=O,T.uniforms.uFront.value=P,{light:g*(.035+t*.65+z*.2),flash:z*g,ignition:y}}}}const $e=a=>a<0?0:a>1?1:a,V=(a,e,o)=>$e((a-e)/(o-e));function za(a,e=256){const o=document.createElement("canvas");o.width=o.height=e;const s=o.getContext("2d"),u=s.createRadialGradient(e/2,e/2,0,e/2,e/2,e/2);a.forEach(([h,d])=>u.addColorStop(h,d)),s.fillStyle=u,s.fillRect(0,0,e,e);const n=new Wt(o);return n.colorSpace=Mt,n}function Ua(a,e,o=1,s=!0){const u=new $t(new qt({map:a,blending:te,depthWrite:!1,depthTest:!0,transparent:!0,opacity:o,fog:s}));return u.scale.set(e,e,1),u}const _e=`
...
`;function le(a,e={},o={}){return new Q({vertexShader:La,fragmentShader:a,uniforms:{uTime:{value:0},...e},...o})}function Ia(a,e){const o=[],s=new R(new ue(6200,32,20),le(Ba,{uTravel:{value:0}},{side:Kt,depthWrite:!1}));s.renderOrder=-10,a.add(s),o.push(s.material);const u=Ye(1977),n=e?2800:7200,h=new Float32Array(n*3),d=new Float32Array(n*3),v=new Float32Array(n);for(let y=0;y<n;y++){h.set([(u()-.5)*6500,(u()-.5)*4100,800-u()*8500],y*3);const P=u(),O=.25+Math.pow(u(),3)*1.6;d.set([O*(P>.7?1:.72),O*(P>.7?.77:.84),O*(P>.7?.51:1)],y*3),v[y]=.65+Math.pow(u(),7)*2.4}const A=new Se;A.setAttribute("position",new oe(h,3)),A.setAttribute("color",new oe(d,3)),A.setAttribute("aSize",new oe(v,1));const E=new Qe(A,new Q({uniforms:{uPixelRatio:{value:1},uTime:{value:0}},vertexShader:`attribute float aSize; varying vec3 vColor; uniform float uPixelRatio; uniform float uTime;
    ...
    #include <tonemapping_fragment>
    #include <colorspace_fragment>
    }`,vertexColors:!0,transparent:!0,depthWrite:!1,blending:te}));a.add(E);const x=new ae;x.position.set(190,25,-180),x.rotation.z=.3;const _=new R(new ue(170,96,64),le(Da,{uOpacity:{value:1}},{transparent:!0}));x.add(_),o.push(_.material),e&&x.position.set(80,150,-180);const m=new R(new bt(207,390,256,1),le(ka,{uInner:{value:207},uOuter:{value:390},uOpacity:{value:1},uPlanetCenter:{value:x.position}},{side:Te,transparent:!0,depthWrite:!1}));m.rotation.x=-Math.PI/2+.17,x.add(m),a.add(x);const{group:T}=pa();T.scale.setScalar(.69),a.add(T),a.add(new Qt(11060713,4600095,1.8));const w=new ut(16770493,3.3);w.position.set(-350,260,400),a.add(w);const S=new ut(7780095,2.5);S.position.set(350,-100,-600),a.add(S);const p=za([[0,"rgba(235,249,255,1)"],[.045,"rgba(151,208,255,.9)"],[.2,"rgba(72,139,255,.18)"],[1,"rgba(0,0,0,0)"]]),c=_a(e),F=c.group;F.position.set(e?35:170,25,-1050),a.add(F);const i=new R(new ce(1100,950),le(xt,{uKind:{value:0},uOpacity:{value:1},uVariant:{value:0}},{transparent:!0,depthWrite:!1,blending:te}));i.position.set(e?35:180,0,-1750),a.add(i),o.push(i.material);const r=Ua(p,170,.9,!1);r.position.copy(i.position),a.add(r);function l(y,P,O,M){const D=new R(new ce(y,y),le(Ha,{uOpacity:{value:1}},{transparent:!0,depthWrite:!1}));return D.position.set(e?P*.22:P,O,M),a.add(D),o.push(D.material),D}const b=l(720,165,30,-2410),f=Oa(e),g=f.group;g.position.set(e?75:235,30,-3090),a.add(g);const t=new ae;a.add(t);const z=[];return[[230,70,-4050,850,.32],[-370,190,-4470,540,-.5],[560,-180,-4490,650,.8],[-150,-80,-4940,1150,-.1]].forEach(([y,P,O,M,D],Z)=>{const J=new R(new ce(M,M*[.72,.46,.95,.58][Z]),le(xt,{uKind:{value:1},uOpacity:{value:.95},uVariant:{value:Z}},{transparent:!0,depthWrite:!1,blending:te}));J.position.set(e?y*.55:y,P,O),J.rotation.z=D,t.add(J),o.push(J.material),z.push(J.material)}),{sky:s,stars:E,planet:x,body:_,rings:m,probe:T,nova:F,supernova:c,remnant:i,pulsar:r,blackHole:b,quasar:g,quasarSystem:f,galaxies:t,galaxyMaterials:z,updateTime(y){o.forEach(P=>{P.uniforms.uTime.value=y})},dispose(){const y=new Set,P=new Set,O=new Set([p]);a.traverse(M=>{M.geometry&&y.add(M.geometry),M.material&&(Array.isArray(M.material)?M.material:[M.material]).forEach(D=>{P.add(D),D.map&&O.add(D.map)})}),y.forEach(M=>M.dispose()),P.forEach(M=>M.dispose()),O.forEach(M=>M.dispose())}}}const Ae={pause:"Pause motion",resume:"Resume motion",fallback:"The voyage is unavailable on this device. All portfolio content is available below.",chapters:["top","now","work","projects","research","stack","education","contact"].map(a=>({id:a}))},qe=a=>Math.max(0,Math.min(1,a)),Na=a=>{const e=qe(a);return e*e*(3-2*e)};function Va(a,e,o=""){const s=j.useRef({report:()=>{}}),u=j.useRef(new WeakSet);return j.useLayoutEffect(()=>{if(!a||matchMedia("(prefers-reduced-motion: reduce)").matches)return;const n=s.current,h=u.current,d=[...document.querySelectorAll(e)],v=[...document.querySelectorAll(o)],A=new Set,E=new Set,x=new Map,_=new Map;let m=null;const T=i=>{i.removeAttribute("data-forge"),i.classList.remove("is-forging"),i.style.removeProperty("--forge-heat"),i.style.removeProperty("--forge-cooling")},w=i=>{clearTimeout(_.get(i)),_.delete(i),x.get(i)?.cancel(),x.delete(i),i.removeAttribute("data-nova-pending"),i.removeAttribute("data-nova-surface"),i.style.removeProperty("--nova-heat"),h.add(i)},S=i=>{if(h.has(i)||x.has(i))return;clearTimeout(_.get(i)),_.delete(i);const r=i.getBoundingClientRect(),l=(m?.x??innerWidth*.8)-(r.left+r.width/2),b=(m?.y??innerHeight*.4)-(r.top+r.height/2),f=Math.hypot(l,b)||1,g=innerWidth<600?32:52;i.setAttribute("data-nova-surface",""),i.style.setProperty("--nova-heat",String(m?.energy??.5));const t=i.animate([{opacity:0,translate:`${l/f*g}px ${b/f*g}px`,scale:"0.975"},{opacity:1,translate:"0px 0px",scale:"1"}],{duration:850,easing:"cubic-bezier(0.16, 1, 0.3, 1)",fill:"both"});x.set(i,t),t.onfinish=()=>w(i)};v.forEach(i=>{h.has(i)||i.getBoundingClientRect().top<innerHeight?h.add(i):i.setAttribute("data-nova-pending","")});const p=new IntersectionObserver(i=>{i.forEach(({target:r,isIntersecting:l})=>{v.includes(r)?l?(E.add(r),!h.has(r)&&!x.has(r)&&_.set(r,setTimeout(()=>S(r),220))):(E.delete(r),clearTimeout(_.get(r)),_.delete(r),x.has(r)&&w(r)):l?A.add(r):(A.delete(r),T(r))})},{threshold:0,rootMargin:"0px 0px -4% 0px"});[...d,...v].forEach(i=>p.observe(i));const c=i=>{const r=i.target.closest(o);r&&w(r)};document.addEventListener("focusin",c);const F=i=>{i.detail?.open&&x.forEach((r,l)=>w(l))};return window.addEventListener("lightgate:change",F),n.report=({active:i,x:r=0,y:l=0,radius:b=0,progress:f=.5,energy:g=1})=>{if(!i||!Number.isFinite(r+l+b)){m=null,d.forEach(T);return}m={x:r,y:l,energy:g};const t=1-Na((f-.7)/.3),z=innerWidth<600?100:170;A.forEach(y=>{const P=y.getBoundingClientRect(),O=Math.hypot(P.left+P.width/2-r,P.top+P.height/2-l),M=Math.exp(-Math.pow((b-O)/z,2))*qe(g)*t;y.setAttribute("data-forge",""),y.classList.add("is-forging"),y.style.setProperty("--forge-heat",M.toFixed(3)),y.style.setProperty("--forge-cooling",(qe(g)*t).toFixed(3))}),E.forEach(S)},()=>{p.disconnect(),document.removeEventListener("focusin",c),window.removeEventListener("lightgate:change",F),_.forEach(clearTimeout),v.forEach(i=>{x.has(i)&&w(i),i.removeAttribute("data-nova-pending")}),d.forEach(T),n.report=()=>{}}},[a,e,o]),s}const ja=["--gravity-x","--gravity-y","--gravity-sx","--gravity-sy","--gravity-angle","--gravity-heat"];function Ga(a){let e=0,o=0;for(let s=a;s;s=s.offsetParent)e+=s.offsetLeft,o+=s.offsetTop,s!==a&&(e+=s.clientLeft,o+=s.clientTop);return{left:e,top:o,width:a.offsetWidth,height:a.offsetHeight}}function Wa(a,e){const o=j.useRef({report:()=>{}});return j.useEffect(()=>{if(!a||matchMedia("(prefers-reduced-motion: reduce)").matches)return;const s=o.current,u=[...document.querySelectorAll(e)],n=new Set,h=new Set,d=new Map;let v=!0,A=!1;const E=w=>{w.removeAttribute("data-ripple"),ja.forEach(S=>w.style.removeProperty(S)),h.delete(w)},x=()=>h.forEach(E),_=new IntersectionObserver(w=>{w.forEach(({target:S,isIntersecting:p})=>{p?n.add(S):(n.delete(S),E(S))})},{rootMargin:"15% 0px"});u.forEach(w=>_.observe(w));const m=()=>{v=!0},T=new ResizeObserver(m);return T.observe(document.documentElement),u.forEach(w=>T.observe(w)),window.addEventListener("resize",m),document.fonts?.ready.then(()=>{A||m()}),s.report=({active:w,x:S,y:p,amplitude:c=0,wavelength:F=120,phase:i=0,energy:r=1})=>{if(!w||!Number.isFinite(S+p+c)){x();return}v&&(u.forEach(f=>d.set(f,Ga(f))),v=!1);const l=Math.max(0,Math.min(1,r));[...n].map(f=>{const g=d.get(f),t=g.left-scrollX+g.width/2-S,z=g.top-scrollY+g.height/2-p,y=Math.hypot(t,z)||1,P=1/(1+y/680),O=Math.sin(y/Math.max(1,F)-i),D=-l*P*(innerWidth<600?11:21)+c*P*O,Z=.024*l*P*O;return{el:f,x:t/y*D,y:z/y*D,sx:1+Z,sy:1-Z,angle:Math.max(-1.05,Math.min(1.05,t/Math.max(180,y)*O*l*.8)),heat:Math.abs(O)*l*P}}).forEach(f=>{const{el:g}=f;g.setAttribute("data-ripple","");const t=g.style,z=g.matches(".research__p, .pub");t.setProperty("--gravity-x",`${z?Math.round(f.x*.4):f.x.toFixed(2)}px`),t.setProperty("--gravity-y",`${z?Math.round(f.y*.4):f.y.toFixed(2)}px`),t.setProperty("--gravity-sx",f.sx.toFixed(4)),t.setProperty("--gravity-sy",f.sy.toFixed(4)),t.setProperty("--gravity-angle",`${f.angle.toFixed(3)}deg`),t.setProperty("--gravity-heat",f.heat.toFixed(3)),h.add(g)})},()=>{A=!0,_.disconnect(),T.disconnect(),window.removeEventListener("resize",m),x(),s.report=()=>{}}},[a,e]),o}const $a="#work h3, #work .entry__date, #work .entry__stack, #projects h3, #projects .flagship__labels, #projects .card__labels",qa="#work .entry",Qa="#research .readout, #research .research__title, #research .research__p, #research .pub, #stack .stack__group",yt=Array.from({length:28},(a,e)=>({angle:e*2.399963,reach:.72+e*17%29/42,depth:.85+e*11%23/21})),Ka=[[0,35,550],[55,60,210],[-40,15,-470],[30,-25,-1170],[-35,45,-1800],[20,-20,-2440],[-35,65,-3130],[80,15,-3650]];function Za({reduced:a,isPhone:e}){const o=j.useRef(null),s=j.useRef(null),[u,n]=j.useState(!1),[h,d]=j.useState(!0),v=j.useRef({paused:!1}),A=j.useRef({wake:()=>{}}),E=Va(h&&!a&&!u,$a,qa),x=Wa(!a&&!u,Qa);j.useEffect(()=>{v.current={paused:u},A.current.wake()},[u]),j.useEffect(()=>{const m=o.current,T=A.current;let w;try{w=new Yt({canvas:m,antialias:!1,powerPreference:"high-performance"})}catch{const U=requestAnimationFrame(()=>d(!1));return()=>cancelAnimationFrame(U)}w.outputColorSpace=Mt,w.toneMapping=wt,w.toneMappingExposure=1.18,w.setClearColor(198156);const S=new Xt,p=new Zt(e?62:48,1,.5,11e3),c=Ia(S,e),F=new Jt(Ka.map(U=>new I(...U)),!1,"catmullrom",.32),i=new ra(w);i.addPass(new na(S,p));const r=new fe(new k(1,1),.24,.35,.9);i.addPass(r);const l=new Ge(fa);i.addPass(l);const b=new Ge(ha);i.addPass(b),i.addPass(new ua);const f=s.current,g=[...f.querySelectorAll(".voyage-ejecta")],t={raf:0,last:0,time:0,t:0,target:0,anchors:[],readingBoxes:[],width:1,height:1,dirty:!0,lost:!1,gateOpen:document.documentElement.classList.contains("light-gate-open"),frames:0,totalMs:0,dpr:Math.min(devicePixelRatio||1,e?1.25:1.5)},z=new k,y=new k,P=new k,O=new I,M=new I,D=new I,Z=new ta,J=new ea,Be=()=>{const U=window.scrollY;let L=0;for(let H=0;H<t.anchors.length-1;H++)U>=t.anchors[H]&&(L=H);const B=t.anchors[L]||0,C=t.anchors[L+1]||1;t.target=$e((L+$e((U-B)/Math.max(1,C-B)))/7),t.dirty=!0,!t.raf&&!document.hidden&&!t.lost&&!t.gateOpen&&(t.raf=requestAnimationFrame(de))},Ct=[...document.querySelectorAll(".section__content, .hero__role, .hero__statement")],De=()=>{t.readingBoxes=Ct.map(L=>{const B=L.getBoundingClientRect();return{left:B.left+scrollX,top:B.top+scrollY,width:B.width,height:B.height}});const U=Math.max(1,document.documentElement.scrollHeight-innerHeight);t.anchors=Ae.chapters.map((L,B)=>B===0?0:Math.min(U,Math.max(0,(document.getElementById(L.id)?.getBoundingClientRect().top||0)+scrollY-72)));for(let L=1;L<8;L++)t.anchors[L]=Math.max(t.anchors[L],t.anchors[L-1]+1);Be()},Pe=()=>{t.width=innerWidth,t.height=innerHeight,w.setPixelRatio(t.dpr),w.setSize(t.width,t.height),i.setPixelRatio(t.dpr),i.setSize(t.width,t.height),c.stars.material.uniforms.uPixelRatio.value=t.dpr,p.aspect=t.width/t.height,p.updateProjectionMatrix(),De()},Xe=U=>{z.set((U.clientX/t.width-.5)*2,(U.clientY/t.height-.5)*2)},Ze=()=>z.set(0,0),Je=new ResizeObserver(De);Je.observe(document.documentElement),window.addEventListener("resize",Pe),window.addEventListener("scroll",Be,{passive:!0}),window.addEventListener("pointermove",Xe,{passive:!0}),document.addEventListener("pointerleave",Ze);let ke=!1;document.fonts?.ready.then(()=>{ke||De()}),Pe(),t.t=t.target;function de(U){if(t.raf=0,ke||document.hidden||t.lost||t.gateOpen)return;const L=Math.min((U-(t.last||U))/1e3,.05);t.last=U;const B=a||v.current.paused,C=B?t.target:t.t+(t.target-t.t)*(1-Math.exp(-L*14));t.t=C,B||(t.time+=L);const H=t.time;if(!B||t.dirty){t.dirty=!1,F.getPoint(C,p.position),y.lerp(B?P:z,1-Math.exp(-L*2.5)),p.position.x+=y.x*7,p.position.y-=y.y*5,O.set(p.position.x+12+Math.sin(C*9)*16,p.position.y-13,p.position.z-600),p.lookAt(O),p.rotateZ(Math.sin(C*10)*.025),p.fov=(e?62:48)+Math.sin(C*Math.PI)*3,p.updateProjectionMatrix(),c.sky.position.copy(p.position),c.sky.material.uniforms.uTravel.value=C,c.updateTime(H),c.body.rotation.y=H*.012,c.planet.visible=C<.245,c.body.material.uniforms.uOpacity.value=1-V(C,.2,.245),c.rings.material.uniforms.uOpacity.value=1-V(C,.2,.245);const ot=140+Math.sin(C*14)**2*120+C*50,it=ot*Math.tan(Y.degToRad(p.fov/2)),_t=e?.5:.65;D.set(p.position.x+it*p.aspect*(_t+Math.sin(C*17)*.18),p.position.y-it*(.37+Math.sin(C*12)*.19),p.position.z-ot),c.probe.position.copy(D),c.probe.position.y+=Math.sin(H*.22)*1.5,J.set(.64+Math.sin(C*8)*.3,.3+C*1.5,-.55+Math.sin(C*12)*.25),Z.setFromEuler(J),c.probe.quaternion.copy(Z),c.probe.scale.setScalar((e?.38:.65)*(1-V(C,.83,1)*.75)),c.probe.visible=C<.96;const K=V(C,.2,.44),ie=V(K,.34,.42)*(1-V(K,.82,1)),Pt=Math.exp(-Math.pow((K-.377)/.027,2));c.nova.visible=C>.16&&C<.48,c.nova.visible&&c.supernova.update(K,H,p),c.remnant.visible=C>.42&&C<.55,c.remnant.material.uniforms.uOpacity.value=V(C,.42,.46)*(1-V(C,.49,.55)),c.pulsar.visible=c.remnant.visible,c.remnant.quaternion.copy(p.quaternion),c.remnant.rotateZ(H*.006),c.pulsar.material.opacity=.7+Math.sin(H*1.4)*.12,c.blackHole.visible=C>.46&&C<.69,c.blackHole.material.uniforms.uOpacity.value=V(C,.46,.51)*(1-V(C,.64,.69)),c.blackHole.quaternion.copy(p.quaternion),c.blackHole.rotateZ(-.12);const st=C>.645&&C<.875,rt=c.quasarSystem.update(H,p,st?V(C,.645,.82):-1,a,1-V(C,.8,.875));c.quasar.visible=st,document.documentElement.style.setProperty("--quasar-light",B?"0":rt.light.toFixed(3)),c.galaxies.visible=C>.755,c.galaxyMaterials.forEach(G=>{G.uniforms.uOpacity.value=V(C,.755,.84)*.95}),r.strength=.2+ie*.12+Pt*.18+(B?0:rt.flash*.14),l.enabled=!1,f.style.setProperty("--event-alpha","0"),f.dataset.event="none";const Ie=V(C,.53,.675),Re=Math.sin(Ie*Math.PI),nt=Ie*18-H*1.2;let ve=0,me=0,ge=0;if(K>0&&K<1){M.copy(c.nova.position).project(p),ve=Y.clamp((M.x*.5+.5)*t.width,-t.width*.3,t.width*1.3),me=Y.clamp((-M.y*.5+.5)*t.height,-t.height*.3,t.height*1.3);const G=Math.max(240,Math.abs(p.position.z-c.nova.position.z)),N=t.height/(2*Math.tan(Y.degToRad(p.fov/2))*G);ge=c.supernova.screenRadius(K)*N,B||(f.dataset.event="supernova",f.style.setProperty("--event-x",`${ve.toFixed(1)}px`),f.style.setProperty("--event-y",`${me.toFixed(1)}px`),f.style.setProperty("--event-radius",`${ge.toFixed(1)}px`),f.style.setProperty("--event-alpha",String(ie*.65)),l.enabled=ie>.01,l.uniforms.uKind.value=1,l.uniforms.uCenter.value.set(ve/t.width,1-me/t.height),l.uniforms.uRadius.value=ge/t.height,l.uniforms.uEnergy.value=ie)}const Rt=!B&&ie>.01;if(g.forEach((G,N)=>{if(!Rt){G.style.opacity="0";return}const $=yt[N],xe=$.angle+H*.014,lt=ge*$.reach;G.style.transform=`translate3d(${(ve+Math.cos(xe)*lt).toFixed(1)}px,${(me+Math.sin(xe)*lt*.76).toFixed(1)}px,0) scale(${$.depth})`,G.style.opacity=String(ie*.68)}),!B&&K>0&&K<1?E.current.report({active:!0,x:ve,y:me,radius:ge,progress:K,energy:ie,time:H}):E.current.report({active:!1}),!B&&Re>.01){M.copy(c.blackHole.position).project(p);const G=(M.x*.5+.5)*t.width,N=(-M.y*.5+.5)*t.height;l.enabled=!0,l.uniforms.uKind.value=2,l.uniforms.uCenter.value.set(G/t.width,1-N/t.height),l.uniforms.uEnergy.value=Re,l.uniforms.uPhase.value=nt,x.current.report({active:!0,x:G,y:N,amplitude:Re*(e?11:22),wavelength:t.height/29,phase:nt,progress:Ie,energy:Re,time:H})}else x.current.report({active:!1});if(l.uniforms.uAspect.value=p.aspect,b.enabled=!0,b.enabled){const G=t.readingBoxes.filter(N=>N.top+N.height>scrollY-90&&N.top<scrollY+t.height+90).slice(0,3);b.uniforms.uFeather.value.set(85/t.width,64/t.height);for(let N=0;N<3;N++){const $=G[N],xe=b.uniforms[`uRect${N}`].value;$?xe.set(($.left-scrollX-96)/t.width,1-($.top-scrollY+$.height+80)/t.height,($.left-scrollX+$.width+96)/t.width,1-($.top-scrollY-80)/t.height):xe.set(-2,-2,-2,-2)}}i.render(),m.dataset.ready="true",!B&&L>0&&(t.frames++,t.totalMs+=L*1e3,t.frames===120&&(t.totalMs/120>22&&t.dpr>.8&&(t.dpr=Math.max(.8,t.dpr-.25),Pe()),t.frames=0,t.totalMs=0))}!B&&!t.raf&&(t.raf=requestAnimationFrame(de))}T.wake=()=>{t.dirty=!0,t.last=0,!t.raf&&!document.hidden&&!t.gateOpen&&(t.raf=requestAnimationFrame(de))};const He=()=>{cancelAnimationFrame(t.raf),t.last=0,!document.hidden&&!t.gateOpen&&(t.dirty=!0,t.raf=requestAnimationFrame(de))},et=U=>{t.gateOpen=!!U.detail?.open,cancelAnimationFrame(t.raf),t.raf=0,t.last=0,t.gateOpen?(E.current.report({active:!1}),x.current.report({active:!1}),document.documentElement.style.setProperty("--quasar-light","0"),f.style.setProperty("--event-alpha","0")):T.wake()};window.addEventListener("lightgate:change",et);const tt=U=>{U.preventDefault(),t.lost=!0,document.documentElement.style.setProperty("--quasar-light","0"),cancelAnimationFrame(t.raf),t.raf=0,E.current.report({active:!1}),x.current.report({active:!1}),f.style.setProperty("--event-alpha","0"),g.forEach(L=>{L.style.opacity="0"}),d(!1)},at=()=>{t.lost=!1,t.dirty=!0,d(!0),He()};return document.addEventListener("visibilitychange",He),m.addEventListener("webglcontextlost",tt),m.addEventListener("webglcontextrestored",at),t.raf||(t.raf=requestAnimationFrame(de)),()=>{ke=!0,document.documentElement.style.removeProperty("--quasar-light"),T.wake=()=>{},cancelAnimationFrame(t.raf),Je.disconnect(),window.removeEventListener("resize",Pe),window.removeEventListener("lightgate:change",et),window.removeEventListener("scroll",Be),window.removeEventListener("pointermove",Xe),document.removeEventListener("pointerleave",Ze),document.removeEventListener("visibilitychange",He),m.removeEventListener("webglcontextlost",tt),m.removeEventListener("webglcontextrestored",at),c.dispose(),r.dispose(),i.passes.forEach(U=>{U!==r&&U.dispose?.()}),i.dispose(),w.dispose()}},[a,e,E,x]);const _=document.getElementById("motion-control");return q.jsxs(q.Fragment,{children:[q.jsx("canvas",{ref:o,className:"voyage","aria-hidden":"true"}),q.jsx("div",{className:"voyage-scrim","aria-hidden":"true"}),q.jsx("div",{className:"voyage-vignette","aria-hidden":"true"}),q.jsxs("div",{className:"voyage-effects",ref:s,"aria-hidden":"true",children:[q.jsx("div",{className:"voyage-front"}),yt.map((m,T)=>q.jsx("i",{className:"voyage-ejecta"},T))]}),h&&!a&&_&&Et.createPortal(q.jsx("button",{className:"voyage-motion",type:"button","aria-label":u?Ae.resume:Ae.pause,"aria-pressed":u,onClick:()=>n(m=>!m),children:q.jsx("span",{"aria-hidden":"true",children:u?"▷":"Ⅱ"})}),_),!h&&q.jsx("p",{className:"visually-hidden",role:"status",children:Ae.fallback})]})}export{Za as default};
