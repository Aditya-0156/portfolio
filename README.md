...
- A quasar that ignites from a copper ember: its disk gathers, the core heats, one ignition pulse fires, and three-dimensional bipolar jets grow from the core over four seconds. Its blue pulse reaches the Stack content before giving way to spiral, barred and elliptical galaxies.
- Filmic tone mapping and restrained bloom.

The supernova's projected shock front also drives the page: work emerges once from the explosion and cools through gold and copper. Each content block settles even if scrolling stops. Body copy remains solid and unblurred; headings and metadata take on a warm, opaque tint. The black hole shares its wave phase with the document, pulling, stretching and releasing nearby research. Sections use one left-aligned reading column with transparent project surfaces. The repeated sidebar labels, metadata columns and scene title cards are removed; the navigation and accessible document headings identify the sections. A GPU pass compresses bright scenery only beneath visible reading areas, with feathered edges and no panel fill. Brighter text and restrained shadows retain the scene behind the copy.

The 3D journey stays behind the portfolio without chapter captions or a bottom control panel. A discreet pause button in the navigation stops continuous canvas rendering while leaving scrolling available. Each work entry forms once from the supernova and stays settled; revisiting the section does not replay its entrance.
...
Animation uses elapsed time rather than a fixed per-frame interpolation factor. General scroll entrances keep text at full opacity and scale; the three work entries use one brief formation fade. Anchor navigation applies the header offset once; wheel scrolling has a short tail, while touch scrolling remains native. Phone rendering starts with fewer stars and explosion particles and a lower pixel ratio. Sustained slow frames reduce rendering resolution further. The explosion expands in a vertex shader instead of rewriting particle positions on the CPU.

The animation loop stops when the tab is hidden or the light dialog is open. Pause and reduced motion render only when scrolling, resizing or changing a relevant control. Reduced motion also disables the text forge, layout ripple, smooth scroll and animated reveals. Without WebGL, portfolio content and links remain available. Losing a graphics context clears the content effects so content cannot remain hidden.
...
Portfolio copy lives in `src/content`. Internal journey anchors live in `src/content/voyage.js`. Career facts and the resume were preserved during the visual rebuild.
...
- `src/lib/voyage/readingPass.js`: soft highlight compression beneath visible reading areas.
